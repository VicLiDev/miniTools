...
# 关闭 git 对非 ASCII 文件名（如中文）的转义：默认 true 会把中文显示成形如 "\346\265\213" 的
# 八进制串，个人开发环境设 false 直接显示原始中文更友好。
# 注意：false 适合人眼看 git status；若用脚本/管道解析文件名（含空格/引号）则保持 true 更安全。
# 前提：终端/locale 需为 UTF-8，否则中文照样乱码。
