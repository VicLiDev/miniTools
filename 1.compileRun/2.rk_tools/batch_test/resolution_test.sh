...
test_spec_rsl()
{
    ...
    do
        ...
        [ "${cur_sp}" == "jpeg" ] && frm_cnt=1 || frm_cnt=10
        ...
        if [ -e ${enc_out_norm_path} ]; then
            [ "`stat -c %s ${enc_out_norm_path}`" -eq 0 ] && enc_norm_res="faile"
        fi
        ...
        if [ -e ${enc_out_kmpp_path} ]; then
            [ "`stat -c %s ${enc_out_kmpp_path}`" -eq 0 ] && enc_kmpp_res="faile"
        fi
        ...
        if [ "${enc_norm_res}" == "pass" ]; then
            ...
            if [ -e ${dec_out_path} ]; then
                [ "`stat -c %s ${dec_out_path}`" -eq 0 ] && dec_res="faile"
            fi
